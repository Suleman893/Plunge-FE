  ...
  return (
    <>
      ...
      <Popper
        ...
        placement={leftAlignMenu ? 'top-start' : 'top-start'}
        transition
        disablePortal
        sx={{ zIndex: 1 }}
                ...
                <MenuList autoFocusItem={open}>
                  {options.map((option: OptionType, index: number) => {
                    if (typeof option === 'string') {
                      return (
                        <MenuItem key={index} onClick={handleClose} className='text-sm'>
                          {option}
                        </MenuItem>
                      )
                      ...
                      const { isToolTip, toolTipText, ...menuItemProps } = option.menuItemProps
                      ...
                      const menuItem = (
                        <MenuItem
                          ...
                          onClick={e => {
                            handleClose(e)
                            menuItemProps && menuItemProps.onClick ? menuItemProps.onClick(e) : null
                          }}
                        >
                      ...
                      return isToolTip && toolTipText ? (
                        <Tooltip title={toolTipText} key={index} placement='left'>
                          <span>{menuItem}</span>
                        </Tooltip>
                      ) : (
                        menuItem
